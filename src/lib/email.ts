...
/**
 * El destinatario SIEMPRE sale de la configuracion del servidor.
 * Nunca del cuerpo de la peticion: si el navegador pudiera elegirlo,
 * cualquiera podria usar el dominio para enviar correo a terceros.
 */
const TO_EMAIL = import.meta.env.CONTACT_TO_EMAIL ?? import.meta.env.TO_EMAIL;
...
// Suficientemente estricto para descartar basura, sin rechazar direcciones validas.
...
export async function sendContactEmail(data: ContactFormData): Promise<SendResult> {
  ...
  try {
    ...
    if (error) {
      console.error("[contact] Resend rechazo el envio:", error.message);
